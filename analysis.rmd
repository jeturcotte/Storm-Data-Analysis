...
# produce a means of interpreting the EXP columns
...
# get and summarize crop damage data
...
# get and summarize poperty damage data
...
# get and summarize human damage data
human_damage <- casualties %>% group_by(EVTYPE) %>% summarize(
...
With the above, we can finally take a look at the results.

# Results

## Property Damage
```{r graphing property damage}
p <- ggplot(prop_damage[prop_damage$amount>0,], aes(reorder(event,amount,max),amount))
p + stat_summary(fun.y='mean',geom='bar',aes(fill=measure),position="dodge") + coord_flip() + scale_y_log10(labels=scales::dollar) + labs(title="Property Damage by Type of Event",y="Dollar Amount",x="Event Type")
```
...
## Crop Damage
```{r graphing crop damage}
p <- ggplot(crop_damage[crop_damage$amount>0,], aes(reorder(event,amount,max),amount))
p + stat_summary(fun.y='mean',geom='bar',aes(fill=measure),position="dodge") + coord_flip() + scale_y_log10(labels=scales::dollar) + labs(title="Crop Damage by Type of Event",y="Dollar Amount",x="Event Type")
```
