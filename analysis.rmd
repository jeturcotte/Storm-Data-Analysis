...
So we have entries referring to a flood and to a blizzard.  All the rest mention thunderstorms.  Many, however, including the flood-only reference ALSO reference 'later entries.'  In other words, other rows.  Counting both brings the danger of double-counting data.  So, let's set aside the blizzard, mark any others that do NOT mention 'additional reports' as thunderstorms, and the rest as to be ignored.

```{r adjust storms data according to summary entries}
storms[storms$REFNUM==summaries[grepl("blizzard",tolower(summaries$REMARKS)),]$REFNUM,]$EVTYPE <- 'blizzard'
storms[storms$REFNUM %in% summaries[!grepl("additional",tolower(summaries$REMARKS)),]$REFNUM,]$EVTYPE <- 'thunderstorm'
