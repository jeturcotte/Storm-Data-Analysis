...
... we have a few more levels with values than merely *K*, *M*, and *B*, BUT these (including the unnamed factor) represent the overwhelming majority of records, so we'll discount the rest, given that this examine is to determine the most costly and the most deadly of weather incident types.  Let's define a means of correcting the costly damage types, and run it against *PROPDMG* and *CROPDMG*.  We'll then toss the old columns.
